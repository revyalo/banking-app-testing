...
| Tarea 2 - Workflow main-deploy | Ricardo Ullco Lagla | Completada |
...
| Tarea 4 - Memoria | Grupo Z | Completada |
...
Durante el desarrollo se usó el siguiente comando para comprobar el estado del repositorio:
...
En la rama `feature-2`, hubo un problema con las pruebas unitarias debido al formato de los mensajes, que se solucionó correctamente.
...
Además, se corrigieron las comprobaciones de mensajes de notificación para adaptarlas al formato decimal utilizado por el servicio. Como ajuste final, se añadió una prueba específica para comprobar que un usuario bloqueado tampoco puede recibir dinero mediante transferencia.

Commit de ajuste final: [76f1db3 - Add banned destination transfer test](https://github.com/revyalo/ais-2026-grupo-z/commit/76f1db3).
...
- Nombre de la aplicación en Azure: `bankingapp-production-urjc`.
- URL pública: https://bankingapp-production-urjc-bhbcebcag5cte5e4.spaincentral-01.azurewebsites.net
- Imagen Docker versionada: [ricardo192004/banking-app:1.2.0](https://hub.docker.com/r/ricardo192004/banking-app/tags?name=1.2.0)
- Imagen Docker `latest`: [ricardo192004/banking-app:latest](https://hub.docker.com/r/ricardo192004/banking-app/tags)

![App desplegada en Azure](images/azure-login.png)
![Dashboard Azure](images/azure-dashboard.png)

[Ejecución del workflow Deploy to Production](https://github.com/revyalo/ais-2026-grupo-z/actions/runs/26451293595)

### Ajustes finales del workflow de despliegue

Para cerrar la práctica se revisó el workflow `main-deploy.yml` y se aplicaron los siguientes ajustes:

- El job `smoke-test` depende ahora de `publish-docker` y `deploy-azure`, por lo que puede usar la versión extraída de `pom.xml`.
- El workflow se ejecuta solo ante cambios reales en código o configuración del proyecto, no ante cambios en `README.md` ni en workflows.
- El despliegue usa el nombre de aplicación asociado al publish profile configurado en GitHub: `bankingapp-production-urjc`.
- El `SmokeTest` recibe la URL desplegada mediante la propiedad Maven `appUrl`.

Nota: si se crea una App Service nueva llamada exactamente `banking-app-production`, será necesario descargar su publish profile desde Azure y sustituir el secret `AZURE_WEBAPP_PUBLISH_PROFILE` en GitHub por el de esa nueva app.

Commit de ajuste: [1dd4366 - Fix CI/CD workflow configuration](https://github.com/revyalo/ais-2026-grupo-z/commit/1dd4366).
